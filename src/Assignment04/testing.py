...
class Viterbi(object):
    ...
    def viterbi(self):
        ...
        actual=nltk.pos_tag(self.observations)
        #print (actual)        
        index=0        
        for i,j in actual:
            #print (j,':',finalPath[index].encode('utf8'))
            if (finalPath[index].encode('utf8')==j):
                Viterbi.correct+=1
            Viterbi.total+=1
            index+=1
        print(Viterbi.total,' : ',Viterbi.correct)
        return toReturnObj
    
    def efficiency(self):
        print ('efficiency:',(float(Viterbi.correct)*100/Viterbi.total))
